<template>
  <div id="album-view">
    <div class="wrapper-header">
      <h1>ALBUMS</h1>
      <div class="settings">
        <button id="btn-grid" @click="toggleGrid" :class="{ active: isGridView }">
          <IconGrid />
        </button>
        <button id="btn-list" @click="toggleList" :class="{ active: !isGridView }">
          <IconList />
        </button>
      </div>
    </div>
    <ul id="list-albums" :class="{ grid: isGridView }">
...
export default {
  ...
  methods: {
    ...
    getSongText(number) {
      return number === 1 ? number + " song" : number + " songs";
    },
    selectAlbum(album) {
    ...
    album() {
      ...
      this.allSongs.forEach(song => {
        ...
        song.album.artists.forEach(artist => {
          if (!albums[song.album.id].artists.some(test => test.id === artist.id)) {
